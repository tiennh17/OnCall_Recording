...
def download(url,path):
    try:
        ...
        print (error)
        f_out.write("-------download fail recording name: " + url + " with errror---" + (str)(error) + "\n")
        return False

def getToken(user_name,password):
    try:
        f_out.write(timenow().strftime("%Y-%m-%d %H:%M:%S") + "-----Start get token tenant :" + user_name + "-----\n")
...
def connectMinIO(server,access_key, secret_key):
    f_out.write(timenow().strftime("%Y-%m-%d %H:%M:%S") + "\n-----Start Connect MinIO server-----\n")
    client = Minio(
        #"118.68.171.191:9000",
        #"play.min.io",
        server,
...
if __name__ == "__main__" :
    ...
    global list_path
    tmp_pathStorage = ""
    tmp_pathStorage = "/usr/src/app"
    f_out = open("/usr/src/app/log_recording.txt","a+") 
    #tmp_pathStorage = "/home/OnCall/recording/"
    #f_out = open("/home/TienNH17/OnCall_Recording/log_recording.txt","a+") 
    global db_recording
    list_path = []
    #clientMinIO = connectMinIO("118.68.171.191:9000","Q3AM3UQ867SPQQA43P2F","zuf+tfteSlswRu7BJ86wekitnifILbZam1KYY3TY")
    clientMinIO = connectMinIO("10.10.35.94:9000","Oncalluser","OncallMini0")
    ...
    for db in dbs :
        if db[0:2] == 'FO':#DB dich vu OnCall
            ...
            f_out.write(timenow().strftime("%Y-%m-%d %H:%M:%S") + "-----Start writting record database for tenant :" + InfoTenant[0] + "-----\n")
            for page_record in list_record :
                insert_record = db_recording.oncall_record.insert_many(page_record)
            f_out.write(timenow().strftime("%Y-%m-%d %H:%M:%S") + "-----Start downloading file recording for tenant :" + InfoTenant[0] + "-----\n")
            for record in list_path :
                ...
                if (download(url,tmp_pathStorage + str(record[1]))) == False:
                    ...
                    count += 1
                    putFileMinIO(clientMinIO,"oncall3",str(record[1]),tmp_pathStorage + str(record[1]))
            print("\nso luong recording download success: ",count)
            f_out.write("so luong recording download success for tenant " + InfoTenant[0] + ": " + (str)(count) + "\n")
        else:
            continue
